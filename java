JVM(java virtual machine) -> is really important for java to run. Java is platform independent..whereas jvm is platform dependent which means we have to install jvm in our machine 
  based on windows, linux and mac os. jvm will run only bytecode . so, javaCode -> Compiler -> byteCode -> JVM . we have 100 or 1000 of files to run . but , JVM will run only one file .
